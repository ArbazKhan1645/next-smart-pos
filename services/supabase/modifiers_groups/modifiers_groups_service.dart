...
class ModifiersGroupsService {
  // Constants
  static const String _TABLE = 'modifier_group';
  static const int _DEFAULT_BATCH_SIZE = 1000;
  static const Duration _CONNECTION_TIMEOUT = Duration(seconds: 30);

  // Services and clients
  final SupabaseClient _supabase = Supabase.instance.client;
  final Logger _logger = Logger('modifier_groupService');
  ...
  /// Insert a new modifier_group Modifier
  Future<SupabaseResult> insertmodifierGroupModifier(
  ...
  /// Update an existing modifierGroup Modifier
  Future<SupabaseResult> updatemodifierGroup(
    String uuid,
    ...
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'modifierGroup Modifier UUID cannot be empty',
      );
    }
  ...
  /// Delete a modifierGroup by UUID
  Future<SupabaseResult> deletemodifierGroup(String uuid) async {
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'modifierGroup Modifier UUID cannot be empty',
    ...
    return _executeQuery(() async {
      ...
      return 'modifierGroup deleted successfully';
    });
  }

  /// Get a modifierGroup Modifier by UUID
  Future<SupabaseResult> getModifierGroupByUUID(String uuid) async {
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'modifierGroup UUID cannot be empty',
      );
    }
  ...
  /// Get modifierGroup  with pagination
  Future<SupabaseResult> getAllModifierGroup({
  ...
  /// Get count of modifierGroup
  Future<int> _getmodifierGroupCount(int locationId) async {
    try {
      ...
      _logger.warning('Failed to get modifierGroup count: $e');
      return 0;
    }
  }

  /// Main entry point for syncing modifierGroup Modifier

  /// Sync modifierGroup in the background with proper error handling
  Stream<SyncProgress> syncModifierGroupInIsolate() async* {
    ...
    try {
      ...
      totalCount = await _getmodifierGroupCount(locationId);

      // If no modifierGroup exist, return early with success
      if (totalCount == 0) {
        yield SyncProgress(0, 0, []);
        return;
      }
      ...
      while (hasMore) {
        final dynamicList = await getAllModifierGroup(
          limit: batchSize,
          offset: offset,
        ...
        if (dynamicList.success && dynamicList.data != null) {
          ...
          final List<ModifierGroups> modifierGroupModelToSave = [];

          // Process each modifierGroup in the batch
          for (var json in dataList) {
            try {
              // Create table data object
              final modifierGroupModel = ModifierGroups.fromJson(json);

              // Add to batch insert list
              modifierGroupModelToSave.add(modifierGroupModel);
            } catch (e) {
              errors.add(
                'Error parsing modifierGroup: ${json['name'] ?? 'Unknown'}, reason: $e',
              );
            }
          }

          // Batch save modifierGroup for better performance
          if (modifierGroupModelToSave.isNotEmpty) {
            try {
              await database.modifiersGroupRepository.createOrUpdateAll(
                modifierGroupModelToSave,
              );
              progress += modifierGroupModelToSave.length;
            } catch (e) {
              errors.add('Error bulk saving modifierGroup: $e');
            }
          }
