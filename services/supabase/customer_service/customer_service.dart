...
class CustomersService {
  ...
  final Logger _logger = Logger('CustomersService');
  ...
  Future<SupabaseResult> insertCustomer(
  ...
  Future<SupabaseResult> updateCustomer(
    String uuid,
    ...
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'Customer UUID cannot be empty',
      );
    }
  ...
  Future<SupabaseResult> deletecustomers(String uuid) async {
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'Customer UUID cannot be empty',
  ...
  Future<SupabaseResult> getcustomersByUUID(String uuid) async {
    if (uuid.isEmpty) {
      return SupabaseResult(
        success: false,
        error: 'Customer UUID cannot be empty',
  ...
  /// Get all customers
  Future<SupabaseResult> getAllcustomers() async {
    return _executeQuery(() async {
  ...
  Future<SupabaseResult> getcustomersByLocation(String location) async {
    if (location.isEmpty) {
      return SupabaseResult(
        success: false,
  ...
  /// Get customers of a location with pagination
  Future<SupabaseResult> getAllcustomersOfLocation({
  ...
  /// Get count of customers for a location
  Future<int> _getcustomersCount(int locationId) async {
    try {
      ...
      _logger.warning('Failed to get customers count: $e');
      return 0;
    }
  }

  /// Main entry point for syncing customers
  Stream<SyncProgress> syncAllcustomersFromSupabase() async* {
    yield* synccustomersInIsolate();
  }

  /// Sync customers in the background with proper error handling
  Stream<SyncProgress> synccustomersInIsolate() async* {
    ...
    try {
      ...
      totalCount = await _getcustomersCount(locationId);

      // If no customers exist, return early with success
      if (totalCount == 0) {
        yield SyncProgress(0, 0, []);
        return;
      }
      ...
      while (hasMore) {
        final dynamicList = await getAllcustomersOfLocation(
          limit: batchSize,
          offset: offset,
        ...
        if (dynamicList.success && dynamicList.data != null) {
          ...
          final List<customers> customersToSave = [];

          // Process each Customer in the batch
          for (var json in dataList) {
            try {
              // Create table data object
              final customer = customers.fromJson(json);

              // Add to batch insert list
              customersToSave.add(customer);
            } catch (e) {
              errors.add(
                'Error parsing customers: ${json['name'] ?? 'Unknown'}, reason: $e',
              );
            }
          }

          // Batch save customers for better performance
          if (customersToSave.isNotEmpty) {
            try {
              ...
              errors.add('Error bulk saving customers: $e');
            }
          }
