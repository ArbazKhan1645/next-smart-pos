  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '40px 20px' }}>
        {step === 'location' && <PickLocation ctx={ctx} setCtx={setCtx} onNext={onNext}/>}
...
const PickLocation = ({ ctx, setCtx, onNext }) => {
  const [sel, setSel] = React.useState(ctx.location);
  return (
    <>
      <FlowHeader
        title="Select your location"
        sub={`Maya · You have access to ${DATA.locations.length} locations across 8 cities. Pick where you'll be working today.`}
      />
