...
const TABLE = 'modifier_display_levels_location';
const BATCH_SIZE = 1000;
const CONNECTIVITY_CACHE_MS = 5000;

export class ModifierDisplayLevelsLocationsService {
  constructor() {
  ...
  async getAll({ limit = BATCH_SIZE, offset = 0, locationId } = {}) {
  ...
  async *syncFromSupabase(db) {
    ...
    try {
      ...
      while (hasMore) {
        const result = await this.getAll({ limit: BATCH_SIZE, offset, locationId });
        if (result.success && result.data) {
          const items = result.data;
          if (items.length) {
            try { await db.modifierDisplayLevels.createOrUpdateAll(items); progress += items.length; }
...
export const modifierDisplayLevelsLocationsService = new ModifierDisplayLevelsLocationsService();
