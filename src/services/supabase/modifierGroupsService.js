...
export class ModifierGroupsService {
  ...
  async insertModifierGroup(data) {
  ...
  async updateModifierGroup(uuid, updateData) {
  ...
  async deleteModifierGroup(uuid) {
    ...
    return this._executeQuery(async () => {
      ...
      if (error) throw error;
      return 'Modifier group deleted successfully';
    });
  }

  async getModifierGroupByUUID(uuid) {
    if (!uuid) return { success: false, error: 'UUID cannot be empty' };
    return this._executeQuery(async () => {
  ...
  async getAllModifierGroups({ limit = BATCH_SIZE, offset = 0, locationId } = {}) {
  ...
  async *syncFromSupabase(db) {
    ...
    try {
      ...
      while (hasMore) {
        const result = await this.getAllModifierGroups({ limit: BATCH_SIZE, offset, locationId });
        if (result.success && result.data) {
          const items = result.data;
          if (items.length) {
            try { await db.modifierGroups.createOrUpdateAll(items); progress += items.length; }
